	...
	if (event.data && event.data.indexOf('action') !== -1) {
		...
		switch (payload.action) {
			case 'unread':
				...
				localStorage['asana-plus-notifications-totalUnread'] = window.asanaPlusTotalUnread;
...
document.addEventListener('paste', function (event) {
	...
	var hi = document.querySelectorAll('.hidden-file-input');
...
document.addEventListener('keydown', function (event) {
	...
	var button, tempItem, closeDialogElement;
	...
	if (event.altKey && event.keyCode > 48 && event.keyCode < 52) {
		...
		if (window.__inReactMode) {
			item = ([].slice.call(document.querySelectorAll('.topbar a')).slice(1,3) || [])[event.keyCode - 49];
		} else {
			item = (document.querySelectorAll('#navigation_dock_domain_view_main .photo-atm-and-feed .list-item') || [])[event.keyCode - 49];
		}
	...
	if (window.__inReactMode) {
		if (document.querySelector('.topbar-navButton')) {
			document.querySelector('.topbar-navButton').setAttribute('title', 'Command (or ctrl) + .');
		}
	...
	// tabl = hi
	if (event.keyCode === 9  && document.body.classList.contains('asana-plus-expand')) {
		hideArea();
	}

	// Esc too (not working now)
	if (event.keyCode == 27 && document.body.classList.contains('asana-plus-expand')) {
	...
	function isValidArea (element) {
		return element
			&& (element.id === 'property_sheet:details_property_sheet_field:description'
				// Ignore this for now (buggy)
				|| (0 && element.contentEditable && element.classList.contains('generic-input')));
	}
...
window.addEventListener('load', function (event) {
	...
	var currentCount, baseIcon, interval = 1000, can, c, soundElement;

	// Will come in super handy
	window.__inReactMode = document.querySelector('.react-app-node') ? true : false;
	...
	function nodeInsertCheck (event) {
        ...
        document.querySelector('#collapsed_label_description').dispatchEvent(evt);

				// Focus back
        setTimeout(function () {
          document.querySelector('[id="property_sheet:quick_add_property_sheet_field:name"]').click();
				  document.querySelector('[id="property_sheet:quick_add_property_sheet_field:name"]').focus();
        }, 60);
			}, 0);
		}
	}
	...
	['animationstart'].forEach(function (e)  { document.addEventListener(e, nodeInsertCheck); });
   ...
   // })
	...
	function placeholderChecker  () {
		...
		var tmp = document.querySelector('#nav_search_input');
	...
	function getCount () {
		...
		if (!window.__inReactMode) {
			...
			var source = document.querySelector('.inbox-count');
			...
			var hasNewNotifications = document.querySelector('.topbar-notificationsButton.has-newNotifications') ? true : false;
			...
			return  Math.max(parseInt(localStorage['asana-plus-notifications-totalUnread']), window.asanaPlusTotalUnread) || 1;
		}
	...
	function draw () {
		...
		function loaded () {
			...
			if (cnt > 0) {
				...
				c.fillText(txt, txt.length > 1 ? 3 : (window.__inReactMode && !window.asanaPlusTotalUnread ? 7 : 8), 9);
			}
		...
		function updateInboxLink () {
			...
			var element = document.querySelector('.topbar-notificationsButton');
...
function postToNotificationsFrame (action, data) {
	...
	if (!window.asanaNotificatonsFrame) {
		window.asanaNotificatonsFrame = document.getElementById('asana-notifications-frame');
	}

	window.asanaNotificatonsFrame.postMessage(JSON.stringify({
		action: 	action,
		data: 	data
	}), 'https://' + location.host);
}
