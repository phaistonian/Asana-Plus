...
function poll () {
	...
	var entries 	= getEntries(), events = [], maxLatestId, lastEvent;
...
function getDataFromElement (entry, id, ts) {
	...
	var creator 		= entry.querySelector('.blockStoryView-actors a'),
	...
	var creatorName = getContent(creator),
	...
	return {
		...
		creatorIcon: creatorIcon, // cache them please
		content: content,
	...
	function getCreatorIconByName (input) {
		...
		var c, ctx, img, name = getNameByInput(input);
		...
		var x = c.width/2 - parseInt(ctx.measureText(name).width)/2;
		var y = c.height/2 + c.width/2/2 - 4;
...
function getEntries () {
	...
	var q = document.querySelectorAll('.inbox-thread-wrapper .story.unread:not(.notified)'), entries = [], index = 0;

	if (!q || !q.length) {
		return 0;
	}

	$A(q).forEach(function (element) {
		entries.push({
			index: 	index++,
...
function notify (title,  event, options) {
	...
	var notification = new window.NotificationNative(title, options);
...
function parseDate (input, notAsTS) {
	...
	var matches = input.match(/^(.*?) at (.*?):(.*?)(am|pm)$/),
		DT = new Date(),
	...
	var result = new Date(year, month, day, hour, mins);
